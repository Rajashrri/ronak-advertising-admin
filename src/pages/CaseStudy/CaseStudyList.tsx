...
export default function CaseStudyList() {
  ...
  const fetchCaseStudies = async () => {
    try {
      ...
      const response = await getCaseStudiesApi(page, limit, search);
  ...
  const handleStatus = async (id: string) => {
    try {
      const response = await changeCaseStudyStatusApi(id);
  ...
  const handleDelete = async (id: string) => {
    ...
    try {
      const response = await deleteCaseStudyApi(id);
  ...
  return (
    <>
      <PageBreadcrumb pageTitle="Case Study List" />

      <div className="space-y-6">
        <ComponentCard title="Case Study List">
          <div className="overflow-hidden rounded-2xl border border-gray-200 bg-white shadow-sm dark:border-gray-700 dark:bg-gray-900">
            <div className="flex flex-col gap-4 px-6 py-5 sm:flex-row sm:items-center sm:justify-between">
              ...
              <div className="w-full sm:w-80">
                <input
                  type="text"
                  placeholder="Search industry or case study..."
                  value={search}
                  onChange={(e) => {
                      ...
                      >
                        ...
                        <TableCell className="px-6 py-4">
                          <div className="flex items-center justify-center gap-2 flex-wrap">
                            ...
                            <Link
                              to={`/case-study-testimonial/${item._id}`}
